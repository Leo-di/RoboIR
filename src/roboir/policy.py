...
class RuleBasedPolicy:
    ...
    def decide(self, scene_graph: SceneGraph, affordance_map: AffordanceMap, task_frame: TaskFrame | None = None) -> PolicyDecision:
        if not self.planner.registry.skills:
            return PolicyDecision(candidate=None, reason="no registered skills")
        candidate = self.planner.best(scene_graph, list(affordance_map.affordances), task_frame=task_frame)
